 ...
 * > 3. 优先等级  原有 header > header > attribute > paramter
...
public class GrayDeployTransmitProerties {
     ...
     * <注意> 默认传递 String 和 基本类型    对象需要实现接口 {@link HeadTransmit} 并 注册为bean
    ...
    /**
     * 向后续服务传递当前服务请求中的paramter
     * <注意> 默认传递 String 和 基本类型    对象需要实现接口 {@link HeadTransmit} 并 注册为bean
     * <注意> paramter 会传递到 后续服务请求中的 header 里
     * <注意> 如果请求中有相同名称的header 将不会传递 ( 使用feign中带有的header值 )
     * <注意> 如果 paramters和headers指定的相同名称 优先取header中值 header 无值时才取 paramters中值  ifnull(header,paramter)
     */
